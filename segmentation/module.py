...
from segmentation.ops import Conv_layer2D, SCmodule_block, EncoderDecoderConvLSTM

class Generator_VGG16_mini_SE(nn.Module):
    
    def __init__(self, dims=3, in_channels=2, out_channels=1, gf_dim=64, kernel_size=3):
        ...
        self.output_layer = Conv_layer2D(dims, gf_dim*2, out_channels, strides=1, act=Act.SIGMOID, is_transposed=True) 
...
class Generator_middle(nn.Module):
    ...
    def __init__(self, dims=3, in_channels=128, out_channels=1, gf_dim=64, kernel_size=3):
        ...
        self.output_layer = Conv_layer2D(dims, gf_dim, out_channels, strides=1, act=Act.SIGMOID, is_transposed=True) 
...
class BILSTM_SC_UNET(nn.Module):
    ...
    def __init__(self, dims=3, in_channels=2, out_channels=1, kernel_size=3, pool_kernel=2, act="relu", generator_model=generators["VGG16_mini_SE"]):
        
        super(BILSTM_SC_UNET, self).__init__()
        
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        self.generator = generator_model(dims=3, in_channels=2, out_channels=1, gf_dim=64, kernel_size=3).to(device)
        self.middle_generator = Generator_middle(dims=3, in_channels=128, out_channels=1, gf_dim=64, kernel_size=3).to(device)
        self.BiLSTM = EncoderDecoderConvLSTM(nf=64, in_chan=64, out_C=1).to(device)
